...
import 'package:fl_chart/src/chart/horizontal_bar_chart/horizontal_bar_chart_data.dart';
...
class HorizontalBarChartPainter extends AxisChartPainter<HorizontalBarChartData>
  ...
  HorizontalBarChartPainter(HorizontalBarChartData data, HorizontalBarChartData targetData,
  ...
  void paint(Canvas canvas, Size size) {
    ...
    final List<double> verticalGroups = calculateGroupValues(size, data.barGroups, data.alignment);
    groupBarsPosition = calculateGroupAndBarsPosition(verticalGroups, data.barGroups);
  ...
  List<double> calculateGroupValues(
    ...
    switch (alignment) {
      ...
      case BarChartAlignment.end:
        ...
        for (int i = barGroups.length - 1; i >= 0; i--) {
          ...
          verticalGroups[i] = (topTextSpace + drawSize.height) - tempValue - group.width / 2;
          tempValue += group.width + data.groupsSpace;
        }
        break;

      case BarChartAlignment.center:
        double sumHeight = barGroups.map((group) => group.width).reduce((a, b) => a + b);
        ...
        for (int i = 0; i < barGroups.length; i++) {
          final group = barGroups[i];
          verticalGroups[i] = topTextSpace + verticalMargin + tempValue + group.width / 2;
      ...
      case BarChartAlignment.spaceBetween:
        final double sumHeight = barGroups.map((group) => group.width).reduce((a, b) => a + b);
  ...
  List<GroupBarsPosition> calculateGroupAndBarsPosition(
    ...
    for (int i = 0; i < barGroups.length; i++) {
      ...
      barGroup.barRods.asMap().forEach((barIndex, barRod) {
        ...
        barsValues.add(groupValue - (barGroup.width / 2) + tempValue + widthHalf);
  ...
  void drawBars(Canvas canvas, Size viewSize, List<GroupBarsPosition> groupBarsPosition) {
    ...
    for (int i = 0; i < data.barGroups.length; i++) {
      ...
      for (int j = 0; j < barGroup.barRods.length; j++) {
        ...
        final cornerLeft = max(borderRadius.topLeft.x, borderRadius.bottomLeft.x);
        final cornerWidth = cornerLeft + max(borderRadius.topRight.x, borderRadius.bottomRight.x);
        final right = barRod.y != 0 ? max(getPixelX(barRod.y, drawSize), cornerWidth) : left;

        /// Draw [BackgroundBarChartRodData]
        if (barRod.backDrawRodData.show && barRod.backDrawRodData.y != 0) {
          final backRight = max(getPixelX(barRod.backDrawRodData.y, drawSize), cornerWidth);
        ...
        if (barRod.y != 0) {
          ...
          if (barRod.rodStackItem != null && barRod.rodStackItem.isNotEmpty) {
            for (int i = 0; i < barRod.rodStackItem.length; i++) {
              ...
              final stackTop = max(getPixelY(stackItem.toY, drawSize), cornerWidth);
        ...
        if (barRod.showTitle && (barRod.title?.isNotEmpty ?? false)) {
          ...
          if (contentWidth <= 0 ||
            ...
            final span = TextSpan(style: barRod.sideTitleStyle, text: barRod.title);
            tp = TextPainter(
                text: span,
                textAlign: TextAlign.left,
                textDirection: TextDirection.ltr,
            ...
            final spaceRight = drawSize.width - contentWidth - barRod.titlePadding - cornerWidth;
            if (spaceRight > 0) {
    ...
    if (topTitles.showTitles) {
      ...
      while (verticalSeek <= data.maxX) {
        ...
        final TextPainter tp = TextPainter(
            ...
            textScaleFactor: textScale);
        tp.layout();
        x = getPixelX(verticalSeek, drawSize) - (verticalSeek * tp.width / data.maxX);
        tp.paint(canvas, Offset(x, getTopOffsetDrawSize() - topTitles.margin - tp.height));
  ...
  void drawTouchTooltip(
    ...
    final Offset mostTopOffset = Offset(
      getPixelX(showOnRodData.y, chartUsableSize),
    ...
    final Rect rect = Rect.fromLTWH(
        min(mostTopOffset.dx + tooltipData.tooltipBottomMargin, viewSize.width - tooltipWidth),
  ...
  BarTouchedSpot _getNearestTouchedSpot(
    ...
    for (int i = 0; i < groupBarsPosition.length; i++) {
      ...
      for (int j = 0; j < groupBarPos.barsOffset.length; j++) {
        ...
        final double barMaxX = getPixelX(targetData.barGroups[i].barRods[j].y, chartViewSize);
        final double barMinY = barY - barWidth / 2;
        final double barMaxY = barMinY + barWidth;

        final double backDrawBarMaxX =
            getPixelX(targetData.barGroups[i].barRods[j].backDrawRodData.y, chartViewSize);
        final EdgeInsets touchExtraThreshold = targetData.barTouchData.touchExtraThreshold;

        final bool isXInBarBounds = (barMinX - touchExtraThreshold.left <= touchedPoint.dx) &&
            (touchedPoint.dx <= barMaxX + touchExtraThreshold.right);

        final bool isYInTouchBounds = (barMinY - touchExtraThreshold.top <= touchedPoint.dy) &&
        ...
        final bool isXInBarBackDrawBounds =
            (barMinX - touchExtraThreshold.left <= touchedPoint.dx) &&
                (touchedPoint.dx <= backDrawBarMaxX + touchExtraThreshold.right);

        final bool isXInTouchBounds =
            (targetData.barTouchData.allowTouchBarBackDraw && isXInBarBackDrawBounds) ||
                isXInBarBounds;
        ...
        if (isXInTouchBounds && isYInTouchBounds) {
          ...
          final nearestSpotPos = Offset(getPixelX(nearestSpot.x, chartViewSize), barY);
  ...
  @override
  bool shouldRepaint(HorizontalBarChartPainter oldDelegate) => oldDelegate.data != data;
}
